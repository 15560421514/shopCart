...
export default {
  ...
  methods: {
    ...
    handler:throttle(async function(type, disNum, cart){
      ...
      try {
        //这里的await是等待这句请求是否成功
        await this.$store.dispatch('reqAddOrUpdateShopCart',{ skuId:cart.skuId, skuNum:disNum });
        //修改成功再一次获取服务器数据进行展示
        this.getData();
      } catch (error) {
  ...
  computed: {
    ...
    // 计算全勾
    isAllCheck() {
      //遍历数组原理 只要全部元素isChecked属性都为1   返回true
      //只要有一个不为1  返回false
